...
Веб-интерфейс — разбор решения без чтения логов:

```bash
python app.py
```

Откроется `http://localhost:8000`. На одном экране:

- метрики прогона и сравнение со стартовым примером организаторов;
- **кнопка «Сравнить стратегии»** — считает цену главного архитектурного решения: перенос
  вывода на сегмент даёт **+12%** к медиане (4 324 552 против 3 862 777) и охват 7 210 против 6 035;
- **кнопка «Устойчивость ×5»** — прогон на пяти seed с графиком разброса, чтобы было видно,
  что знак результата не скачет;
- **разбор данных** — почему агент сознательно не трогает 59% базы: медианный прирост по
  сегментам и лучшие целевые тарифы по истории;
- результат каждого пилота на графике, красным — отброшенные гипотезы;
- план кампаний и обоснование.

Seed меняется прямо в интерфейсе.

**Веб не нужен для сдачи.** Основной сценарий (`local_eval.py`, `make_submission.py`) работает без
него и без интернета. Интерфейс ничего не считает сам — он запускает ту же `Agent.act` и показывает,
что агент измерил.
...
4. `python make_submission.py` — создаётся `submission.csv` с двумя кампаниями.

Ожидаемый результат последнего шага:

| campaign_name | filter_arpu_segment | target_tariff | channel |
|---|---|---|---|
