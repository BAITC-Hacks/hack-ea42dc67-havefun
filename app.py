...
# Обработчики глушат stdout через redirect_stdout и временно меняют
# настройку стратегии в compare. Оба действия глобальны для процесса,
# поэтому два одновременных запроса могли смешать вывод и настройки.
# Один замок на все вычислительные обработчики — просто и надёжно;
# прогон занимает секунды, очередь для аналитика незаметна.
#
# RLock, а не Lock: обработчики вызывают друг друга внутри одного потока
# (ask и plan.csv опираются на run, ask ещё и на insights), и обычный
# замок давал бы дедлок на первом же вложенном вызове.
...
class RunRequest(BaseModel):
    ...
    Настройки стратегии намеренно НЕ принимаются из запроса: раньше они
    писались в глобальные переменные модуля, и следующий запрос работал
    с чужими значениями — результат зависел от истории нажатий.
    Стратегия одна и та же для всех, меняется только seed.
    """
...
def run(req: RunRequest) -> dict:
    """Прогон агента на мок-среде."""
    with _RUN_LOCK:
        if not 0 <= req.seed <= 10_000:
            raise HTTPException(status_code=422,
                                detail="seed должен быть целым от 0 до 10000")
...
def _check_seed(seed: int) -> None:
    """Единая проверка seed для всех обработчиков.

    Раньше отрицательный seed доходил до numpy и превращался в HTTP 500.
    Отказ во вводе должен быть отказом во вводе, а не падением сервиса.
    """
    if not isinstance(seed, int) or isinstance(seed, bool) or not 0 <= seed <= 10_000:
        raise HTTPException(status_code=422,
                            detail="seed должен быть целым от 0 до 10000")
...
def ask(req: AskRequest) -> dict:
    ...
    with _RUN_LOCK:
        ...
        facts.append("Сегменты с отрицательным приором по умолчанию в план не берутся, "
                     "но вслепую не отбрасываются: часть пилотов резервируется именно на "
                     "них, и если замер покажет прибыльность, сегмент попадёт в план. "
                     "Данные синтетические, это симулятор кейса, а не реальная база Beeline.")
        # Вопрос про урезанный бюджет система умеет считать, а не оценивать
        # на глаз. Передаём готовый пересчёт, чтобы модель отвечала числом,
        # а не отказом «по фактам нельзя определить».
        try:
            for b in (25_000, 50_000):
                w = whatif(b)
                facts.append(
                    f"При бюджете {b:,} у.е. агент пересобирает план заново: "
                    f"чистый результат {w['net']:,.0f}, тратит {w['cost']:,.0f}, "
                    f"охват {w['customers']:,} абонентов.")
            facts.append("Охват при разных бюджетах почти не меняется: он упирается "
                         "в лимит контактов, а не в деньги. Бюджет определяет канал, "
                         "а канал — долю согласившихся.")
        except Exception as exc:  # noqa: BLE001 — без пересчёта ответ просто беднее
            print(f"[ask] пересчёт по бюджету недоступен: {type(exc).__name__}")
...
def compare() -> dict:
    ...
    with _RUN_LOCK:
        out = {}
        saved = A.TRANSFER_TO_SEGMENT          # вернём как было в любом случае
        try:
...
def whatif(budget: int = 50000) -> dict:
    ...
    with _RUN_LOCK:
        ...
        if not 1_000 <= budget <= 100_000:
            raise HTTPException(status_code=422,
                                detail="budget должен быть целым от 1000 до 100000")
